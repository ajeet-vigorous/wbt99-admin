...
const Basic = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      render: (createdAt) => <span className="gx-text-nowrap">{(moment(createdAt)).format('DD MMM hh:mm:ss A')}</span>,
    ...
    //     {usertype === 'client' ?
    //     <span className='gx-text-nowrap'>{row.balance > 0 ? Number.parseFloat(Math.abs(row.balance)).toFixed(2) + " - Dena" : Number.parseFloat(Math.abs(row.balance)).toFixed(2) + " - Lena"}</span>
    //   :<span className='gx-text-nowrap'>{row.balance > 0 ? Number.parseFloat(Math.abs(row.balance)).toFixed(2) + " - Lena" : Number.parseFloat(Math.abs(row.balance)).toFixed(2) + " - Dena"}</span>}
    // },
    ...
    <>
      ...
      <Card className="gx-card">
        <div className="gx-bg-grey gx-px-3 gx-py-3 gx-bg-flex">
          <span className="gx-fs-lg gx-font-weight-bold gx-text-white gx-align-items-center gx-pt-1 gx-text-uppercase gx-text-nowrap">
            {usertype === 'subadmin' ? "mini admin" : usertype} Transactions
          </span>
          <BackButton />
        </div>
        <div className="gx-px-2 gx-pt-3 ">
          ...
          >
            ...
            <Row className="gx-text-uppercase">
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5 ">
                <Form.Item
                  ...
                  className="gx-w-100"
                >
                  <label className="ant-col ant-form-item-label gx-py-1">{usertype === 'subadmin' ? "mini admin" : usertype} <span className="gx-text-red">*</span></label>
              ...
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                <Form.Item name="collection" rules={[{ required: true }]}>
                  <label className="ant-col ant-form-item-label gx-py-1">{"collection"} <span className="gx-text-red">*</span></label>

                  <Select disabled getPopupContainer={trigger => trigger.parentElement} defaultValue="CASH A/C">
              ...
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                <Form.Item name="date" rules={[{ required: true }]}>
                  <label className="ant-col ant-form-item-label gx-py-1">{"Date"} <span className="gx-text-red">*</span></label>
                  <DatePicker disabled className="gx-mb-3 gx-w-100 gx-border-redius0"
              ...
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                ...
                <Form.Item name="amount" rules={[{ required: true }]}>
                  <label className="ant-col ant-form-item-label gx-py-1">{"Amount"} <span className="gx-text-red">*</span></label>
                  <Input
                    type="number"
                    placeholder="Amount"
                    className="gx-border-redius0"
              ...
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                <Form.Item name="paymentType" rules={[{ required: true }]}>
                  <label className="ant-col ant-form-item-label gx-py-1">{"Payment Type"} <span className="gx-text-red">*</span></label>
                  <Select
                    showSearch
              ...
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                ...
                <Form.Item name="remark" rules={[{ required: true }]}>
                  <label className="ant-col ant-form-item-label gx-py-1">{"Remark"} <span className="gx-text-red">*</span></label>
                  <Input
                    placeholder="Remark"
                    className="gx-border-redius0"
            ...
            <Row>
              <Col lg={8} md={8} sm={12} xs={24} className="gx-px-5">
                <Form.Item name="ledgerType"
                // rules={[{ required: true }]}
                >
                  <label className="ant-col ant-form-item-label gx-py-1">{"Ledger Type"} </label>
                  <Select
                    showSearch
                    placeholder="All"
                    optionFilterProp="children"
            ...
            <Row className=" gx-mb-5 gx-px-4">
              <Col span={24}>
                <Button type="primary" onClick={() => onFinish()} className="gx-border-redius gx-px-3">
                  Submit
                </Button>
              </Col>
            </Row>
      ...
      {loading ? <div style={{
        ...
        zIndex: 10
      }} className=" gx-left-0 gx-w-100 gx-h-100 gx-bg-flex gx-align-items-center  gx-justify-content-center ">
        ...
        <>
          {showRow && <>
            ...
            <Card className="gx-w-100 gx-py-4 ">
              <Row className="gx-px-4">



                <Col xl={6} lg={6} md={22} sm={22} xs={22} className="gx-pt-3 " >
                  <Form.Item name="paymentType1" rules={[{ required: true }]}>
                    <label className="ant-col ant-form-item-label gx-py-1">{"Payment Type"} <span className="gx-text-red">*</span></label>
                    <Select
                      showSearch
                      placeholder="Select PaymentType"
                      optionFilterProp="children"
                      onChange={value => handleInputChange('paymentType1', value)}
                      // getPopupContainer={trigger => trigger.parentElement}
                      filterOption={(input, option) =>
                        option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                      }
                ...
                <Col xl={16} lg={16} md={22} sm={22} xs={22} className="gx-pt-5 gx-px-3 gx-bg-flex gx-justify-content-end gx-align-items-center">
              ...
              <Row className="gx-py-5">
                <Col xl={6} lg={6} md={22} sm={22} xs={22} className=" gx-px-3 gx-bg-flex gx-justify-content-lg-center gx-justify-content-start" >
                  <div className="gx-fs-lg gx-px-1  gx-text-danger">
                ...
                <Col xl={6} lg={6} md={22} sm={22} xs={22} className="gx-px-3 gx-bg-flex gx-justify-content-lg-center gx-justify-content-start   ">
                  <div className="gx-fs-lg   gx-text-green">
                ...
                {usertype === 'client' ?
                  <Col xl={6} lg={6} md={22} sm={22} xs={22} className=" gx-px-3 gx-bg-flex gx-justify-content-lg-center gx-justify-content-start">
                    <span className={`gx-fs-lg  gx- text-black ${-1 * calAmount > 0 ? "gx-text-green" : "gx-text-danger"} `}>
                      Balance: {Number.parseFloat(Math.abs(calAmount ? calAmount : 0)).toFixed(2)}
                      ({-1 * calAmount > 0 ? "Lena" : "Dena"})
                    </span>
                  </Col> :
                  <Col xl={6} lg={6} md={22} sm={22} xs={22} className=" gx-px-3 gx-bg-flex gx-justify-content-lg-center gx-justify-content-start">
                    <span className={`gx-fs-lg  ${calAmount > 0 ? "gx-text-green" : "gx-text-danger"} `}>
              ...
              <Table rowKey={record => record?.key} className="gx-table-responsive gx-text-uppercase" columns={columns} dataSource={userLadger} bordered pagination={false} size="small"

                rowClassName={(row, index) => index % 2 !== 0 && row.ledgerType != 'settle' ? 'gx-bg-light-grey' : row.ledgerType == 'settle' ? 'gx-bg-yellow' : ''}
              />
